...
const Day = () => {
  ...
  const handleSubmitData = async e => {
    ...
    if (!userData.calories && !userData.steps && !userData.sleep && !userData.mood) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100">
      ...
      <p className="text-black w-150 m-5 text-center bg-blue-200 rounded-lg p-2">
        Здесь ты можешь сохранить информацию о своем дне: сколько шагов ты прошёл, сколько часов спал, как себя чувствуешь и сколько калорий потребил.
        Эти данные помогут тебе отслеживать прогресс и заботиться о себе каждый день.
      </p>
      <form className="bg-white p-8 rounded shadow-md w-full max-w-sm">
        <h2 className="text-2xl font-bold mb-4 text-black">Мой день</h2>
        ...
        <input
          ...
          className="w-full p-2 border mb-4 border-black rounded-lg text-black"
        />
        <select name="mood" value={userData.mood} onChange={handleChangeData} className="w-full p-2 border mb-4 border-black rounded-lg text-black">
        ...
        <button onClick={handleSubmitData} className="w-full bg-green-500 p-2 rounded-lg hover:bg-green-600 cursor-pointer text-white">
          Сохранить
        </button>
        <button onClick={ShowDailyData} className="block mx-auto w-50 bg-green-500 m-5 p-2 rounded-lg hover:bg-green-600 cursor-pointer text-white">
          Посмотреть все записи
        </button>
        <p className={`${message.error ? 'text-red-500' : 'text-green-500'} mt-4 text-center`}>{message.text}</p>
      </form>
